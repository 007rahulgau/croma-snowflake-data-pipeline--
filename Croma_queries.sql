...
CREATE WAREHOUSE IF NOT EXISTS WH_TRNSFORM
WAREHOUSE_SIZE = XSMALL AUTO_SUSPEND = 60 AUTO_RESUME = TRUE;
CREATE WAREHOUSE IF NOT EXISTS WH_BI
WAREHOUSE_SIZE = XSMALL AUTO_SUSPEND = 60 AUTO_RESUME = TRUE;

// Creating Databbase And Scehmas

CREATE DATABASE IF NOT EXISTS CROMA;
CREATE SCHEMA IF NOT EXISTS CROMA.STG;
CREATE SCHEMA IF NOT EXISTS CROMA.INT;
CREATE SCHEMA IF NOT EXISTS CROMA.PRS;

// Standard CSV file format (matches the generated files)
CREATE OR REPLACE FILE FORMAT CROMA.PUBLIC.FF_CSV_STD
  TYPE = CSV
  FIELD_DELIMITER = ','
  SKIP_HEADER = 1
...
TYPE = EXTERNAL_STAGE
  ...
  ENABLED = TRUE
  AZURE_TENANT_ID = '56a12547-fa74-4336-bcc7-a6290a5a9468'
  STORAGE_ALLOWED_LOCATIONS =('azure://azuresnowflakeaccount.blob.core.windows.net/croma/');

  DESC STORAGE integration AZURE_BLOB_INT;
  
// Creating external stage 

CREATE OR REPLACE STAGE CROMA.STG.AZ_STAGE
  URL = 'azure://azuresnowflakeaccount.blob.core.windows.net/croma/'
  STORAGE_INTEGRATION = AZURE_BLOB_INT
  FILE_FORMAT = CROMA.PUBLIC.FF_CSV_STD;

  
LIST @CROMA.STG.AZ_STAGE/product;
desc stage CROMA.STG.AZ_STAGE;
list @CROMA.STG.AZ_STAGE;
...
CREATE OR REPLACE NOTIFICATION INTEGRATION MY_NOTIFICATION_INT
	  ...
	  AZURE_STORAGE_QUEUE_PRIMARY_URI = 'https://azuresnowflakeaccount.queue.core.windows.net/snowpipequeue'
	  AZURE_TENANT_ID = '56a12547-fa74-4336-bcc7-a6290a5a9468';
...
-- PRODUCT
CREATE OR REPLACE PIPE CROMA.STG.P_PRODUCT
  AUTO_INGEST = TRUE
  INTEGRATION = 'MY_NOTIFICATION_INT'
AS
COPY INTO CROMA.STG.STG_PRODUCT
FROM @CROMA.STG.AZ_STAGE/product
FILE_FORMAT = (FORMAT_NAME = CROMA.PUBLIC.FF_CSV_STD);

select SYSTEM$PIPE_STATUS('P_PRODUCT')


-- EMPLOYEE
CREATE OR REPLACE PIPE CROMA.STG.P_EMPLOYEE
  AUTO_INGEST = TRUE
  INTEGRATION = 'MY_NOTIFICATION_INT'
AS
COPY INTO CROMA.STG.STG_EMPLOYEE
FROM @CROMA.STG.AZ_STAGE/employee
FILE_FORMAT = (FORMAT_NAME = CROMA.PUBLIC.FF_CSV_STD);
...
//customer
COPY INTO CROMA.STG.STG_CUSTOMER
FROM @CROMA.STG.AZ_STAGE/customer
FILE_FORMAT = (FORMAT_NAME = CROMA.PUBLIC.FF_CSV_STD)
...
//sales
COPY INTO CROMA.STG.STG_SALES
FROM @CROMA.STG.AZ_STAGE/sales
FILE_FORMAT = (FORMAT_NAME = CROMA.PUBLIC.FF_CSV_STD)

//inventory
COPY INTO CROMA.STG.STG_INVENTORY
FROM @CROMA.STG.AZ_STAGE/inventory
FILE_FORMAT = (FORMAT_NAME = CROMA.PUBLIC.FF_CSV_STD)

//stores
COPY INTO CROMA.STG.STG_STORE
FROM @CROMA.STG.AZ_STAGE/store
FILE_FORMAT = (FORMAT_NAME = CROMA.PUBLIC.FF_CSV_STD)

// DIMENSIONS //
USE DATABASE CROMA;
USE SCHEMA CROMA.INT

//Dimension tables are created in Snowflake during data transformation to provide business context for analysis, improve query performance, and simplify data access for business users. 
//This approach, known as dimensional modeling, is a foundational technique in data warehousing. 
...
//In Snowflake, fact tables are created during data transformation to support the dimensional modeling approach, which is crucial for efficient analytics and business intelligence (BI). 
//This process involves organizing transformed, business-ready data into a structured format that is optimized for reporting and analysis rather than day-to-day transaction processing. 

CREATE OR REPLACE TABLE FACT_SALES (  // contains all the aggregeted values.
...
MERGE INTO CROMA.INT.DIM_EMPLOYEE d
USING (
  SELECT DISTINCT
    EMPLOYEE_ID, FIRST_NAME, LAST_NAME, EMAIL, PHONE,
    HIRE_DATE, TERMINATION_DATE, STORE_ID, JOB_TITLE
  FROM CROMA.STG.STG_EMPLOYEE
) s
...
MERGE INTO CROMA.INT.FACT_SALES t
USING (
  SELECT
  ...
  FROM CROMA.STG.STG_SALES s
  LEFT JOIN CROMA.INT.DIM_STORE    ds ON ds.STORE_ID    = s.STORE_ID
  LEFT JOIN CROMA.INT.DIM_PRODUCT  dp ON dp.PRODUCT_ID  = s.PRODUCT_ID
  LEFT JOIN CROMA.INT.DIM_CUSTOMER dc ON dc.CUSTOMER_ID = s.CUSTOMER_ID
  LEFT JOIN CROMA.INT.DIM_EMPLOYEE de ON de.EMPLOYEE_ID = s.EMPLOYEE_ID
) s
...
MERGE INTO CROMA.INT.FACT_INVENTORY t
USING (
  SELECT
    i.INVENTORY_ID, i.SNAPSHOT_DATE,
    ds.STORE_KEY, dp.PRODUCT_KEY,
    i.ON_HAND_QTY, i.ON_ORDER_QTY, i.REORDER_POINT, i.UNIT_COST
  FROM CROMA.STG.STG_INVENTORY i
  LEFT JOIN CROMA.INT.DIM_STORE   ds ON ds.STORE_ID   = i.STORE_ID
  LEFT JOIN CROMA.INT.DIM_PRODUCT dp ON dp.PRODUCT_ID = i.PRODUCT_ID
) s
...
FROM CROMA.INT.FACT_SALES fs
JOIN CROMA.INT.DIM_STORE   ds ON fs.STORE_KEY   = ds.STORE_KEY
JOIN CROMA.INT.DIM_PRODUCT dp ON fs.PRODUCT_KEY = dp.PRODUCT_KEY
...
FROM CROMA.INT.FACT_INVENTORY fi
JOIN CROMA.INT.DIM_STORE   ds ON fi.STORE_KEY   = ds.STORE_KEY
JOIN CROMA.INT.DIM_PRODUCT dp ON fi.PRODUCT_KEY = dp.PRODUCT_KEY;
...
FROM CROMA.INT.FACT_SALES fs
JOIN CROMA.INT.DIM_CUSTOMER dc ON fs.CUSTOMER_KEY = dc.CUSTOMER_KEY
...
WITH last_snap AS (
  SELECT MAX(SNAPSHOT_DATE) AS max_dt FROM CROMA.INT.FACT_INVENTORY
),
inv AS (
  SELECT fi.STORE_KEY,
         SUM(fi.ON_HAND_QTY * fi.UNIT_COST) AS inventory_value
  FROM CROMA.INT.FACT_INVENTORY fi, last_snap
...
sales_30d AS (
  ...
  FROM CROMA.INT.FACT_SALES fs, last_snap
...
JOIN CROMA.INT.DIM_STORE ds USING (STORE_KEY)
...
-- Base roles
CREATE ROLE IF NOT EXISTS CROMA_ADMIN;
CREATE ROLE IF NOT EXISTS CROMA_ENG;
CREATE ROLE IF NOT EXISTS CROMA_PIPE;
CREATE ROLE IF NOT EXISTS CROMA_ANALYST;
CREATE ROLE IF NOT EXISTS CROMA_POWER_USER;
CREATE ROLE IF NOT EXISTS CROMA_AUDITOR;
CREATE ROLE IF NOT EXISTS CROMA_SUPPORT;

-- Hierarchy: Admin inherits all; SYSADMIN sees admin (optional, org standard)

GRANT ROLE CROMA_ENG         TO ROLE CROMA_ADMIN;
GRANT ROLE CROMA_PIPE        TO ROLE CROMA_ADMIN;
GRANT ROLE CROMA_ANALYST     TO ROLE CROMA_ADMIN;
GRANT ROLE CROMA_POWER_USER  TO ROLE CROMA_ADMIN;
GRANT ROLE CROMA_AUDITOR     TO ROLE CROMA_ADMIN;
GRANT ROLE CROMA_SUPPORT     TO ROLE CROMA_ADMIN;

-- Optionally attach project admin to SYSADMIN (org convention)
GRANT ROLE CROMA_ADMIN TO ROLE SYSADMIN;

-- Run as ACCOUNTADMIN or a role that owns warehouses

GRANT USAGE, OPERATE ON WAREHOUSE WH_INGEST   TO ROLE CROMA_PIPE;
GRANT USAGE, OPERATE ON WAREHOUSE WH_INGEST   TO ROLE CROMA_ENG;

GRANT USAGE, OPERATE ON WAREHOUSE WH_TRNSFORM TO ROLE CROMA_ENG;
GRANT USAGE, OPERATE ON WAREHOUSE WH_TRNSFORM TO ROLE CROMA_ADMIN;

GRANT USAGE ON WAREHOUSE WH_BI TO ROLE CROMA_ANALYST;
GRANT USAGE ON WAREHOUSE WH_BI TO ROLE CROMA_POWER_USER;
GRANT USAGE ON WAREHOUSE WH_BI TO ROLE CROMA_AUDITOR;

-- Run as SYSADMIN (or object owner)
USE DATABASE CROMA;

-- Database USAGE
GRANT USAGE ON DATABASE CROMA TO ROLE CROMA_ANALYST
GRANT USAGE ON DATABASE CROMA TO ROLE CROMA_POWER_USER
CROMA_AUDITOR,  CROMA_ENG,  CROMA_PIPE,  CROMA_ADMIN;

-- Schema USAGE (no object access yet)
GRANT SELECT ON ALL TABLES IN SCHEMA CROMA.STG TO ROLE CROMA_AUDITOR;
GRANT SELECT ON ALL TABLES IN SCHEMA CROMA.STG TO ROLE CROMA_ENG;
GRANT SELECT ON ALL TABLES IN SCHEMA CROMA.STG TO ROLE CROMA_ANALYST;


-- Read raw only for audit/eng; load rights only for pipeline role
GRANT SELECT ON ALL TABLES IN SCHEMA CROMA.STG TO ROLE CROMA_AUDITOR, ROLE CROMA_ENG;
GRANT SELECT ON FUTURE TABLES IN SCHEMA CROMA.STG TO ROLE CROMA_AUDITOR, ROLE CROMA_ENG;
...
-- Future tables
GRANT SELECT ON FUTURE TABLES IN SCHEMA CROMA.STG TO ROLE CROMA_AUDITOR;
GRANT SELECT ON FUTURE TABLES IN SCHEMA CROMA.STG TO ROLE CROMA_ENG;
